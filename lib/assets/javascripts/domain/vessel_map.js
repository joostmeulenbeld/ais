...
Map.prototype.loadMarkers = function() {
  ...
  this.loader.loadMarkers(function(data) {
    ...
    if (self.markerLayer != null) {
		for (var i = 0; i < self.markerLayer.markers.length; i++) {
			var tempmark = self.markerLayer.markers[i];
			self.markerLayer.removeMarker(tempmark);
			tempmark.lonlat = null;
			tempmark.destroy();
		}
      self.markerLayer.destroy();
    }
